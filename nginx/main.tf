...
resource "azurerm_resource_group" "aulacloud" {
  name     = "aulacloud"
  location = "eastus"
}

resource "azurerm_resource_group" "teste-aulacloud" {
  name     = "teste-aulacloud"
  location = "westus2"
}

resource "azurerm_virtual_network" "vnet" {
  name                = "vnet"
  location            = azurerm_resource_group.aulacloud.location
  resource_group_name = azurerm_resource_group.aulacloud.name
...
resource "azurerm_network_interface" "aulacloud-tf" {
  name                = "aulacloud-tf"
  location            = azurerm_resource_group.aulacloud.location
  resource_group_name = azurerm_resource_group.aulacloud.name
...
resource "azurerm_network_interface_security_group_association" "ng-nic-assoc" {
  network_interface_id      = azurerm_network_interface.aulacloud-tf.id
...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  network_interface_ids = [
    azurerm_network_interface.aulacloud-tf.id
  ]
